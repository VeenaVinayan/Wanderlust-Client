...
import { fetchPendingAgentData } from '../../helper/apiHelper';

export const fetchData = async (user: string,page: number =1): Promise< DataResponse | null> => {
  try {
    const response = await axiosInstance.get(`getData/${user}/${PER_PAGE}/${page}`);
...
export const  fetchAllCategory = async (page : number) => {
    try{
      const response = await axiosInstance.get(`/categories/${PER_PAGE}/${page}`);
